...
class MambaBlock(nn.Module):
    ...
    def lowrank_decomp(self, preserve_rate, device, dtype):
        self.proj = random_projection.GaussianRandomProjection(compute_inverse_components=True)
        A_log_proj = self.proj.fit_transform(self.A_log.T.data.cpu().numpy())
        self.A_log = nn.Parameter(torch.from_numpy(A_log_proj).to(device=device, dtype=dtype))
        # self.proj_mat = proj.components_
        # self.inv_proj_mat = proj.inverse_components_
    ...
    def forward(self, x):
        ...
        if self.in_proj is None:
            ...
            x_and_res = self.in_proj(x.half())  # shape (b, l, 2 * d_in)
    ...
    def selective_scan(self, u, delta, A, B, C, D):
        ...
        proj_mat = proj.components_
        delta = delta @ proj_mat.T
        u = u @proj_mat.T
        ...
        y = self.proj.inverse_transform(y)
        print(y.shape)
